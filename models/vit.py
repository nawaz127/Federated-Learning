...
class VisionTransformer(nn.Module):
    """
    Vision Transformer (ViT) model adapted for medical image classification.
    ...
    def __init__(
        ...
        self.vit = create_model(vit_name, pretrained=pretrained, num_classes=0, drop_path_rate=drop_path_rate)  # feature extractor
        ...
        if hasattr(self.vit, 'set_grad_checkpointing'):
            ...
            print("ViT: gradient checkpointing ENABLED (saves ~40% VRAM)")

        # Register hook for XAI
        self._feature_hook_handle = self.vit.norm.register_forward_hook(self.save_features)
    ...
    def _freeze_backbone(self):
        ...
        print("VisionTransformer backbone frozen.")

    def unfreeze_backbone(self):
        for p in self.vit.parameters():
            p.requires_grad = True
        print("VisionTransformer backbone UN-FROZEN.")
    ...
    def forward(self, x: torch.Tensor, return_attention: bool = False):
        # ViT expects 3-channel input
        # Convert 1-channel (grayscale) to 3-channel (RGB) by repeating
        if x.size(1) == 1:
            x = x.repeat(1, 3, 1, 1)

        # timm models do not support output_attentions in forward()
        # For attention-based XAI, use AttentionRollout from utils/xai/ instead
        features = self.vit(x)
        attention_weights = None

        output = self.classifier(features)
        from utils.common_utils import validate_tensor
        output = validate_tensor(output, "VisionTransformer output")
    ...
    def extract_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Ensure not to return attention weights when extracting features
        features = self.vit(x)
        return features
...
def vit_model(num_classes: int = 3, pretrained: bool = True, dropout_rate: float = 0.3, drop_path_rate: float = 0.1, freeze_backbone: bool = False, vit_name: str = "vit_base_patch16_224"):
    """
    Factory function to create the VisionTransformer model.
    """
...
if __name__ == '__main__':
    ...
    model_rgb = vit_model(num_classes=test_num_classes, pretrained=False)
    ...
    model_gray = vit_model(num_classes=test_num_classes, pretrained=False)
    ...
    model_freeze_test = vit_model(num_classes=test_num_classes, pretrained=False, freeze_backbone=True)
